...
namespace shadow {
    ...
    vkutils::PipelineLayout create_opaque_pipeline_layout(const vkutils::VulkanContext& context,
        ...
        if (const auto res = vkCreatePipelineLayout(context.device, &layoutInfo, nullptr, &layout);
            VK_SUCCESS != res) {
            throw vkutils::Error("Unable to create shadow pipeline layout\n"
    ...
    vkutils::Pipeline create_opaque_pipeline(const vkutils::VulkanWindow& window,
        ...
        const vkutils::ShaderModule vert = vkutils::load_shader_module(window, cfg::shadowMapOpaqueVertPath);
        const vkutils::ShaderModule frag = vkutils::load_shader_module(window, cfg::shadowMapOpaqueFragPath);
        ...
        if (const auto res = vkCreateGraphicsPipelines(window.device, VK_NULL_HANDLE,
            ...
            VK_SUCCESS != res) {
            throw vkutils::Error("Unable to create shadow map opaque pipeline\n"
    ...
    vkutils::Pipeline create_alpha_pipeline(const vkutils::VulkanWindow& window,
                                            ...
                                            VkPipelineLayout pipelineLayout) {
        // Load only vertex and fragment shader modules
        const vkutils::ShaderModule vert = vkutils::load_shader_module(window, cfg::shadowMapAlphaVertPath);
        const vkutils::ShaderModule frag = vkutils::load_shader_module(window, cfg::shadowMapAlphaFragPath);
        ...
        if (const auto res = vkCreateGraphicsPipelines(window.device, VK_NULL_HANDLE,
                                                       1, &pipelineInfo, nullptr, &pipeline);
            VK_SUCCESS != res) {
            throw vkutils::Error("Unable to create shadow map alpha pipeline\n"
                                 "vkCreateGraphicsPipelines() returned %s", vkutils::to_string(res).c_str());
        }
    ...
    std::pair<vkutils::Image, vkutils::ImageView> create_shadow_buffer(const vkutils::VulkanWindow& window,
                                                                       const vkutils::Allocator& allocator) {
        const auto [shadowWidth, shadowHeight] = shadowMapExtent;
        vkutils::Image offscreenImage = vkutils::create_image(allocator, shadow::shadowFormat, VK_IMAGE_TYPE_2D,
        ...
        return {
            std::move(offscreenImage),
            vkutils::image_to_view(window, offscreenImage.image, VK_IMAGE_VIEW_TYPE_2D, shadow::shadowFormat,
    ...
    vkutils::Framebuffer create_shadow_framebuffer(const vkutils::VulkanWindow& window,
        ...
        const VkFramebufferCreateInfo framebufferInfo{
            ...
            .attachmentCount = attachments.size(),
            .pAttachments = attachments.data(),
            .width = shadow::shadowMapExtent.width,
            .height = shadow::shadowMapExtent.height,
            .layers = 1
        };
        ...
        if (const auto res = vkCreateFramebuffer(window.device, &framebufferInfo, nullptr, &framebuffer);
            VK_SUCCESS != res) {
            throw vkutils::Error("Unable to create shadow framebuffer\n"
    ...
    void record_commands(VkCommandBuffer commandBuffer,
                         ...
                         VkBuffer sceneUBO,
                         const glsl::SceneUniform& sceneUniform, VkDescriptorSet sceneDescriptorSet,
        ...
        for (const auto& mesh : alphaMeshes) {
            ...
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                    alphaLayout, 1, 1,
                                    &materialDescriptors[mesh.materialId], 0, nullptr);
